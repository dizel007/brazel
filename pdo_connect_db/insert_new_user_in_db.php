...
if ($_POST['password'] !== $_POST['repaet_password']) {
...
$date_write =date('Y-m-d');
  	...
  	$stmt  = $pdo->prepare("INSERT INTO `users` (user_login, user_password, user_active, user_name, user_phone,  ful_name,	user_mobile_phone, user_email, date_write)
                        VALUES (:user_login, :user_password, :user_active, :user_name, :user_phone, :ful_name,	:user_mobile_phone, :user_email, :date_write)");


        $stmt ->bindParam(':user_login', $login);
        $stmt ->bindParam(':user_password', $password);
        $stmt ->bindParam(':user_active', $active_mode);
        $stmt ->bindParam(':user_name', $_POST['user_last_name']);
        $stmt ->bindParam(':user_phone', $main_phone);
        $stmt ->bindParam(':ful_name', $user_ful_name);
        $stmt ->bindParam(':user_mobile_phone', $_POST['user_mobile_phone']);
